...
class VoiceSignalingConsumer(AsyncWebsocketConsumer):
    ...
    def get_redis(cls):
        ...
        if cls._redis is None:
            cls._redis = redis.Redis(host='localhost', port=6379, db=1, decode_responses=True)
        return cls._redis
    ...
    async def connect(self):
        ...
        self.alias = self._get_query_param('alias', 'Anonymous')
    ...
    async def _register_voice_user(self, room_name, alias):
        ...
        print(f'[VoiceSignaling] Usuario {alias} registrado en presencia de sala {room_name}')
    ...
    async def _unregister_voice_user(self, room_name, alias):
        ...
        key = self._get_presence_key(room_name)
        r.hdel(key, alias)
        print(f'[VoiceSignaling] Usuario {alias} desregistrado de presencia de sala {room_name}')
